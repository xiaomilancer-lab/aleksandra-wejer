...
export default function AfterVisitCard({ visit, nextVisit }: { visit: VisitCardData; nextVisit: NextVisitData | null }) {
  ...
  const babyroomUrl = useMemo(() => {
    const url = new URL("/babyroom", "https://aleksandrawejer.pl");
    url.searchParams.set("source", "after-visit-card");
    return url.toString();
  }, []);

  useEffect(() => {
    QRCode.toDataURL(babyroomUrl, {
      ...
      <article className="after-visit-print-page mx-auto w-full max-w-[210mm] overflow-hidden rounded-[28px] border border-[#D5DCCF] bg-white shadow-[0_24px_70px_rgba(45,71,57,0.12)]">
        ...
        <div className="grid gap-7 px-8 py-8 sm:px-12 sm:py-10 lg:grid-cols-[1.15fr_.85fr]">
          <div className="space-y-6">
            ...
            <section>
              <div className="flex items-center gap-3"><Sparkles size={21} className="text-[#B7791F]" /><h2 className="text-xl font-bold">Kącik PsychOLKI bez konta</h2></div>
              <p className="mt-3 text-sm leading-6 text-gray-600">Na stronie czeka bezpłatna przestrzeń do wspólnej zabawy i spokojnego odpoczynku. Nie trzeba się rejestrować ani podawać żadnych danych.</p>
              <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                {["Spokojne szumy", "Kolorowanki i rysowanie", "Memory i puzzle", audience === "parent" ? "Wspólna zabawa z dzieckiem" : "Chwila spokojnej zabawy"].map((item) => <li key={item} className="flex items-start gap-2 rounded-2xl bg-[#EEF1EB] px-4 py-3 text-sm font-semibold"><Check size={17} className="mt-0.5 shrink-0" />{item}</li>)}
              </ul>
            </section>
          ...
          <aside className="rounded-3xl border border-[#D5DCCF] p-6 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#6D7A62]">Bezpłatny Babyroom</p>
            <h2 className="mt-2 text-2xl font-bold">Zeskanuj aparatem</h2>
            <p className="mt-2 text-sm leading-6 text-gray-600">Kod otworzy kącik PsychOLKI bez logowania i bez reklam.</p>
            <div className="mx-auto mt-5 grid aspect-square w-52 place-items-center rounded-3xl border border-[#E5E1D8] bg-white p-3">
              {qrCode ? <Image src={qrCode} width={360} height={360} unoptimized alt="Kod QR do publicznego Babyroomu PsychOLKI" className="h-full w-full" /> : <span className="text-sm text-gray-500">Przygotowywanie kodu…</span>}
            </div>
            <p className="mt-4 break-all text-xs font-semibold text-[#2D4739]">aleksandrawejer.pl/babyroom</p>
            <a href={babyroomUrl} target="_blank" rel="noreferrer" className="after-visit-screen-only mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#6D7A62] px-4 py-2 text-sm font-semibold text-white">Otwórz Babyroom <ExternalLink size={16} /></a>
            <div className="mt-5 rounded-2xl bg-[#294D3D] p-4 text-left text-white"><div className="flex items-center gap-2 font-bold"><Smartphone size={19} />PsychOLKA w telefonie</div><p className="mt-2 text-xs leading-5 text-[#E8EEE5]">Otwórz kącik w telefonie, tablecie albo komputerze. Wszystkie aktywności działają bez zakładania konta.</p></div>
            <div className="mt-6 flex items-start gap-2 rounded-2xl bg-[#F8F5F0] p-4 text-left text-xs leading-5 text-gray-600"><ShieldCheck size={19} className="mt-0.5 shrink-0 text-[#6D7A62]" /><p>Zabawa nie zapisuje danych dziecka. Rysunki i postęp znikają po zamknięciu lub odświeżeniu strony.</p></div>
          </aside>
        </div>
