...
export default function DashboardNewRequests({ requests }: { requests: DashboardRequest[] }) {
  return (
    <DashboardCard className="h-full">
      <div className="flex min-w-0 items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="shrink-0 rounded-2xl bg-[#FFF3DD] p-3 text-[#9A6A19]"><Inbox size={20} aria-hidden="true" /></span><div className="min-w-0"><p className="break-words text-sm text-gray-500">Nowe zgłoszenia</p><h2 className="break-words font-bold text-[#2D4739]">Do potwierdzenia</h2></div></div><span className="shrink-0 rounded-full bg-[#FFF3DD] px-3 py-1 text-sm font-bold text-[#7A6540]">{requests.length}</span></div>
