...
export default function KnowledgeLibraryManager({ materials, privateLibraryAvailable = true }: { materials: KnowledgeMaterial[]; privateLibraryAvailable?: boolean }) {
  ...
  return <div className="space-y-6">
    <section className="rounded-3xl border border-[#E5E1D8] bg-white p-5 shadow-[0_12px_35px_rgba(45,71,57,0.06)] sm:p-7">
      ...
      <div className="relative mt-6"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={20} /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Szukaj: ADHD, mutyzm, żałoba, lęk, para, ćwiczenie do domu..." className="w-full rounded-xl border border-[#D9D6CD] bg-[#F8F5F0] py-3.5 pl-12 pr-4 text-base text-[#2D4739] placeholder:text-[#667085] outline-none focus:border-[#6D7A62] focus:ring-4 focus:ring-[#EEF1EB]" /></div>
