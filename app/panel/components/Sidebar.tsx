...
import { BookOpen, Bot, CalendarClock, CalendarDays, CalendarHeart, CircleHelp, ClipboardList, Flower2, FolderKanban, House, Mail, Menu, Settings, UserRound, Users, X } from "lucide-react";
  ...
  return (
    <>
      <header data-panel-mobile-header className="sticky top-0 z-40 flex w-full min-w-0 max-w-full items-center justify-between border-b border-[#E5E1D8] bg-white/95 px-4 py-3 shadow-sm backdrop-blur lg:hidden">
        ...
        <div className="flex items-center gap-2">{pathname !== "/panel" && <Link href="/panel" aria-label="Wróć do pulpitu" className="inline-flex h-11 min-w-11 items-center justify-center gap-2 rounded-xl border border-[#E5E1D8] px-3 text-sm font-semibold text-[#2D4739]"><House size={20} aria-hidden="true" /><span className="hidden min-[390px]:inline">Pulpit</span></Link>}<button type="button" onClick={() => setMobileOpen(true)} aria-label="Otwórz menu panelu" aria-expanded={mobileOpen} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-[#E5E1D8] text-[#2D4739]"><Menu size={22} aria-hidden="true" /></button></div>
      </header>
