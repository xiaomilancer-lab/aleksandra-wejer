...
export default function PublicBookingAccess() {
  ...
  const showPsychologistPanel = account?.loggedIn && account.role === "psychologist";

  return <>
    <div className="fixed right-6 top-24 z-40 hidden items-center gap-3 md:flex">
      {showPsychologistPanel && <Link href="/panel" className="inline-flex items-center gap-2 rounded-xl bg-white/95 px-4 py-2.5 text-sm font-semibold text-[#4B4338] shadow-lg ring-1 ring-[#E5E2DB] backdrop-blur transition hover:bg-[#EEF1EB] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6D7A62] focus-visible:ring-offset-2"><UserRound size={17} aria-hidden="true" />Wróć do panelu</Link>}
    ...
    {!hideMobile && <div className="fixed inset-x-4 bottom-[max(1rem,env(safe-area-inset-bottom))] z-40 flex gap-2 md:hidden">
      {showPsychologistPanel && <Link href="/panel" aria-label="Wróć do panelu" title="Wróć do panelu" className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-[#6D7A62] bg-white/95 text-[#2D4739] shadow-xl backdrop-blur transition active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6D7A62] focus-visible:ring-offset-2"><UserRound size={19} aria-hidden="true" /></Link>}
