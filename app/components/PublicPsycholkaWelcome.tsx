...
export default function PublicPsycholkaWelcome() {
  ...
  return (
    <section aria-labelledby="public-psycholka-welcome-title" className="relative min-h-screen overflow-hidden bg-[#F9F6F1] px-4 py-10 sm:px-6 sm:py-14">
      <div className="mx-auto flex w-full max-w-5xl flex-col items-center gap-8 text-center">
        {firstVisit ? <PsycholkaGreetingSequence className="public-psycholka-welcome" /> : <PsycholkaWidget context="welcome" action="wave" fallbackAction="greeting" className="public-psycholka-welcome" />}
        <div className="max-w-2xl space-y-5">
          <p className="text-sm font-semibold uppercase tracking-[0.14em] text-[#2F6B5F]">Aleksandra Wejer · psycholog</p>
          <h1 id="public-psycholka-welcome-title" className="font-serif text-4xl font-semibold text-[#23332F] sm:text-5xl">{greeting}</h1>
          <p className="text-lg leading-relaxed text-stone-700 sm:text-xl">Jesteś na stronie Aleksandry Wejer — psychologa dla dzieci, młodzieży, dorosłych, par i rodzin.</p>
          <p className="text-base leading-relaxed text-stone-600 sm:text-lg">Cześć! Jestem {CHARACTER_NAME}. Jeśli chcesz, pomogę Ci odnaleźć się na stronie.</p>
          <p aria-live="polite" className={`min-h-6 text-sm font-semibold text-[#31584F] transition-opacity ${interactionMessage ? "opacity-100" : "opacity-0"}`}>{interactionMessage ?? " "}</p>
          {PSYCHOLKA_DEBUG && <p className="text-xs text-[#6D7A62]">Debug · wylosowany komunikat: {greeting}</p>}
        </div>
        <div className="flex w-full max-w-xl flex-col justify-center gap-3 sm:grid sm:grid-cols-2 sm:items-start">
          <div className="relative inline-flex items-center justify-center">
            ...
            <a href="#kalendarz" onClick={() => showInteractionMessage("❤️ Trzymam kciuki.")} className="relative z-10 inline-flex min-h-12 items-center justify-center rounded-xl bg-[#E63946] px-6 py-3 font-semibold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#cc2f3c] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E63946] focus-visible:ring-offset-2">📅 Umów wizytę</a>
          </div>
          <button type="button" onClick={startGuidedJourney} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-[#2F6B5F] bg-white px-6 py-3 font-semibold text-[#2F6B5F] transition hover:bg-[#F3F8F6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2F6B5F] focus-visible:ring-offset-2">🌿 Rozejrzyj się ze mną</button>
          <PublicQuickContact />
          <a href="https://wa.me/48510777469" target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#25D366] px-6 py-3 font-semibold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#1FAE57] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2">📱 WhatsApp</a>
        </div>
      </div>
      <div id="public-paths" className="mx-auto mt-16 w-full max-w-6xl scroll-mt-24">
