...
export default function HelpAccordion() {
  const [openId, setOpenId] = useState<(typeof categories)[number]["id"] | null>(null);
    ...
    <div>
        ...
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className={`group flex min-h-28 items-center gap-3 rounded-2xl border p-4 text-left transition duration-200 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#DDE5D8] ${isOpen ? "border-[#6D7A62] bg-[#EEF1EB] shadow-sm" : "border-[#E5E1D8] bg-white hover:-translate-y-0.5 hover:border-[#B7C5B0] hover:shadow-md"}`}
            >
      ...
      <div id={contentId} className={`grid transition-[grid-template-rows,opacity,margin] duration-300 ease-out motion-reduce:transition-none ${active ? "mt-4 grid-rows-[1fr] opacity-100" : "mt-0 grid-rows-[0fr] opacity-0"}`}>
        <div className="overflow-hidden">
          {active && <article className="grid gap-5 rounded-3xl border border-[#D5DCCF] bg-[#FCFDFB] p-5 shadow-[0_12px_35px_rgba(45,71,57,0.06)] sm:p-7 lg:grid-cols-[160px_1fr] lg:items-center">
            <div className="flex justify-center"><Image src={active.image} alt="" width={180} height={180} className="h-32 w-32 object-contain sm:h-36 sm:w-36" /></div>
            <div>
              <div className="flex items-center gap-2"><Puzzle size={18} className="text-[#6D7A62]" aria-hidden="true" /><h3 className="text-2xl font-bold text-[#2D4739]">Wsparcie dla: {active.title.toLocaleLowerCase("pl-PL")}</h3></div>
              <p className="mt-3 leading-7 text-gray-700">{active.intro}</p>
              <ul className="mt-4 flex flex-wrap gap-2" aria-label="Przykładowe obszary rozmowy">
                {active.problems.map((problem) => <li key={problem} className="rounded-full bg-[#EEF1EB] px-3 py-1.5 text-sm font-medium text-[#55624D]">{problem}</li>)}
              </ul>
              <p className="mt-5 leading-7 text-gray-600">{active.approach}</p>
