...
export default function PartnerBookingWidget({ partnerId, source }: { partnerId: PartnerId; source?: SourceVariant }) {
  ...
  return <SmartBookingFlow location={config.location} cta={config.cta} showPhone={config.showPhone} showWhatsApp={config.showWhatsApp} skipIntro renderGuide={(state) => <PsycholkaWidget context="welcome" action={state === "greeting" ? "greeting" : state === "booking" ? "point_booking" : state === "waiting" ? "sad" : "happy"} fallbackAction="greeting" className="shrink-0" />} loadAvailability={loadAvailability} submitBooking={async (date, time, contact: BookingContact) => {
