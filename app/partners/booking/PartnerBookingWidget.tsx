...
export default function PartnerBookingWidget({ partnerId, source }: { partnerId: PartnerId; source?: SourceVariant }) {
  ...
  return <SmartBookingFlow location={config.location} cta={config.cta} showPhone={config.showPhone} showWhatsApp={config.showWhatsApp} skipIntro renderGuide={(state) => <PartnerPsycholkaVisual state={state} priority={state === "greeting" || state === "booking"} />} loadAvailability={loadAvailability} submitBooking={async (date, time, contact: BookingContact) => {
